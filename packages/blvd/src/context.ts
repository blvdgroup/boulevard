...
export const getContext = async (): Promise<Context> => {
  ...
  if (process && process.env) {
    ...
    switch (process.env.BLVD_CONTEXT) {
      case 'client': return { type: ContextType.CLIENT, id: await determineId() }
      case 'server': return { type: ContextType.SERVER }
  ...
  return window ? { type: ContextType.CLIENT, id: await determineId() } : { type: ContextType.SERVER }
}
